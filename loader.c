...
#define FLASH_WIP_ERR					(0x1 << 3)
#define FLASH_INIT_ERR					(0x1 << 4)
#define FLASH_ERASE_ERR					(0x1 << 5)
#define FLASH_WRITE_ERR					(0x1 << 6)
#define FLASH_READ_ERR					(0x1 << 7)

#define NUSPI_TXWM_TIMEOUT			(5000)
#define NUSPI_TX_TIMEOUT			(5000)
#define NUSPI_RX_TIMEOUT			(5000)

static void nuspi_read_reg(uint32_t *spi_base, uint32_t offset, uint32_t *value);
static void nuspi_write_reg(uint32_t *spi_base, uint32_t offset, uint32_t value);
...
static int nuspi_init(uint32_t *spi_base);
...
static int flash_wip(uint32_t *spi_base)
{
	...
	retval |= nuspi_tx(spi_base, SPIFLASH_READ_STATUS);
	if (retval != NUSPI_OK) {
...
static int nuspi_init(uint32_t *spi_base)
{
	...
	nuspi_set_dir(spi_base, NUSPI_DIR_TX);
	nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_HOLD);
	retval |= nuspi_tx(spi_base, SPIFLASH_READ_ID);
	if (retval != NUSPI_OK) {
		retval |= FLASH_INIT_ERR;
		goto out;
	}
	...
	for (int i = 0;i < 3;i++) {
		if (NUSPI_OK != nuspi_rx(spi_base, &rx)) {
			retval |= FLASH_INIT_ERR;
			goto out;
		}
		retval |= rx << (i * 8);
...
static int flash_erase(uint32_t *spi_base, uint32_t first_addr, uint32_t end_addr)
{
	...
	uint32_t sector_num = (end_addr - first_addr) / SPIFLASH_SECTOR_SIZE;
	nuspi_disable_hw_mode(spi_base);
	retval |= flash_wip(spi_base);
	if (retval != NUSPI_OK) {
	...
	for (int i = 0;i < sector_num;i++) {
		curr_addr += i * SPIFLASH_SECTOR_SIZE;
		nuspi_set_dir(spi_base, NUSPI_DIR_TX);
		nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_HOLD);
		retval |= nuspi_tx(spi_base, SPIFLASH_WRITE_ENABLE);
		if (retval != NUSPI_OK) {
			retval |= FLASH_ERASE_ERR;
			goto out;
		}
		...
		nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_AUTO);
		nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_HOLD);
		retval |= nuspi_tx(spi_base, SPIFLASH_SECTOR_ERASE);
		if (retval != NUSPI_OK) {
			retval |= FLASH_ERASE_ERR;
			goto out;
		}
		retval |= nuspi_tx(spi_base, curr_addr >> 16);
		if (retval != NUSPI_OK) {
			retval |= FLASH_ERASE_ERR;
			goto out;
		}
		retval |= nuspi_tx(spi_base, curr_addr >> 8);
		if (retval != NUSPI_OK) {
			retval |= FLASH_ERASE_ERR;
			goto out;
		}
		retval |= nuspi_tx(spi_base, curr_addr);
		if (retval != NUSPI_OK) {
			retval |= FLASH_ERASE_ERR;
			goto out;
		}
...
static int flash_write(uint32_t *spi_base, uint8_t* src_address, uint32_t write_offset, uint32_t write_count)
{
	...
	while (write_count > 0) {
		...
		nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_AUTO);
		nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_HOLD);
		retval |= nuspi_tx(spi_base, SPIFLASH_PAGE_PROGRAM);
		if (retval != NUSPI_OK) {
			retval |= FLASH_WRITE_ERR;
			goto out;
		}
		retval |= nuspi_tx(spi_base, write_offset >> 16);
		if (retval != NUSPI_OK) {
			retval |= FLASH_WRITE_ERR;
			goto out;
		}
		retval |= nuspi_tx(spi_base, write_offset >> 8);
		if (retval != NUSPI_OK) {
			retval |= FLASH_WRITE_ERR;
			goto out;
		}
		retval |= nuspi_tx(spi_base, write_offset);
		if (retval != NUSPI_OK) {
			retval |= FLASH_WRITE_ERR;
			goto out;
		}
		for (int i = 0;i < cur_count;i++) {
			retval |= nuspi_tx(spi_base, *(src_address + i));
			if (retval != NUSPI_OK) {
				retval |= FLASH_WRITE_ERR;
				goto out;
			}
...
static int flash_read(uint32_t *spi_base, uint8_t* dst_address, uint32_t read_offset, uint32_t read_count)
{
	...
	nuspi_set_dir(spi_base, NUSPI_DIR_TX);
	nuspi_write_reg(spi_base, NUSPI_REG_CSMODE, NUSPI_CSMODE_HOLD);
	retval |= nuspi_tx(spi_base, SPIFLASH_READ);
	if (retval != NUSPI_OK) {
		retval |= FLASH_READ_ERR;
		goto out;
	}
	retval |= nuspi_tx(spi_base, read_offset >> 16);
	if (retval != NUSPI_OK) {
		retval |= FLASH_READ_ERR;
		goto out;
	}
	retval |= nuspi_tx(spi_base, read_offset >> 8);
	if (retval != NUSPI_OK) {
		retval |= FLASH_READ_ERR;
		goto out;
	}
	retval |= nuspi_tx(spi_base, read_offset);
	if (retval != NUSPI_OK) {
		retval |= FLASH_READ_ERR;
		goto out;
	}
	...
	for (int i = 0;i < read_count;i++) {
		...
		retval |= nuspi_rx(spi_base, (dst_address + i));
		if (retval != NUSPI_OK) {
			retval |= FLASH_READ_ERR;
			goto out;
		}
