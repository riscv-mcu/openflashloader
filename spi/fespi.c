#include "spi.h"

/* Register offsets */
#define FESPI_REG_SCKMODE           (0x04)
#define FESPI_REG_FORCE             (0x0C)
#define FESPI_REG_CSMODE            (0x18)
#define FESPI_REG_FMT               (0x40)
#define FESPI_REG_TXDATA            (0x48)
#define FESPI_REG_RXDATA            (0x4C)
#define FESPI_REG_FCTRL             (0x60)
#define FESPI_REG_FFMT              (0x64)
#define FESPI_REG_IP                (0x74)
#define FESPI_REG_STATUS            (0x7C)
#define FESPI_REG_RXEDGE            (0x80)
/* IP register */
#define FESPI_IP_TXWM               (0x1)
/* FCTRL register */
#define FESPI_FCTRL_EN              (0x1)
#define FESPI_INSN_CMD_EN           (0x1)
/* STATUS register */
#define FESPI_STAT_BUSY             (0x1 << 0)
#define FESPI_STAT_TXFULL           (0x1 << 4)
#define FESPI_STAT_RXEMPTY          (0x1 << 5)
#define FESPI_CSMODE_AUTO           (0)
#define FESPI_CSMODE_HOLD           (2)
#define FESPI_DIR_RX                (0)
#define FESPI_DIR_TX                (1)

#define FESPI_TX_TIMES_OUT          (500)
#define FESPI_RX_TIMES_OUT          (500)

#define RETURN_FESPI_TX_ERROR       (0x1 << 4)
#define RETURN_FESPI_RX_ERROR       (0x1 << 5)

static inline void fespi_read_reg(uint32_t volatile *spi_base, uint32_t offset, uint32_t *value)
{
    *value = spi_base[offset / 4];
}

static inline void fespi_write_reg(uint32_t volatile *spi_base, uint32_t offset, uint32_t value)
{
    spi_base[offset / 4] = value;
}

static inline void fespi_set_dir(uint32_t *spi_base, uint32_t dir)
{
    uint32_t fmt = 0;
    fespi_read_reg(spi_base, FESPI_REG_FMT, &fmt);
    fespi_write_reg(spi_base, FESPI_REG_FMT, (fmt & ~(FESPI_FMT_DIR(0xFFFFFFFF))) | FESPI_FMT_DIR(dir));
}

void spi_init(uint32_t *spi_base)
{
    /* clear rxfifo */
    uint32_t status = 0;
    while (1) {
        nuspi_read_reg(spi_base, FESPI_REG_RXDATA, &status);
        nuspi_read_reg(spi_base, FESPI_REG_STATUS, &status);
        if (status & FESPI_STAT_RXEMPTY) {
            break;
        }
    }
    /* init register */
    nuspi_write_reg(spi_base, FESPI_REG_SCKMODE, 0x0);
    nuspi_write_reg(spi_base, FESPI_REG_FORCE, 0x3);
    nuspi_write_reg(spi_base, FESPI_REG_FCTRL, 0x0);
    nuspi_write_reg(spi_base, FESPI_REG_FMT, 0x80008);
    nuspi_write_reg(spi_base, FESPI_REG_FFMT, 0x30007);
    nuspi_write_reg(spi_base, FESPI_REG_RXEDGE, 0x0);
}

void spi_hw(uint32_t *spi_base, bool sel)
{
    uint32_t fctrl = 0;
    fespi_read_reg(spi_base, FESPI_REG_FCTRL, &fctrl);
    if (sel) {
        fespi_write_reg(spi_base, FESPI_REG_FCTRL, fctrl | FESPI_FCTRL_EN);
    } else {
        fespi_write_reg(spi_base, FESPI_REG_FCTRL, fctrl & ~FESPI_FCTRL_EN);
    }
}

void spi_cs(uint32_t *spi_base, bool sel)
{
    if (sel) {
        fespi_write_reg(spi_base, FESPI_REG_CSMODE, FESPI_CSMODE_HOLD);
    } else {
        fespi_write_reg(spi_base, FESPI_REG_CSMODE, FESPI_CSMODE_AUTO);
    }
}

int spi_tx(uint32_t *spi_base, uint8_t *in, uint32_t len);
{
	uint32_t times_out = 0;
    uint32_t status = 0;
    fespi_set_dir(spi_base, FESPI_DIR_TX);
    for (int i = 0;i < len;i++) {
        times_out = FESPI_TX_TIMES_OUT;
        while (times_out--) {
            fespi_read_reg(spi_base, FESPI_REG_TXDATA, &txfifo);
            if (!(txfifo >> 31)) {
                break;
            }
        }
        if (0 >= times_out) {
            return RETURN_FESPI_TX_ERROR;
        }
        fespi_write_reg(spi_base, FESPI_REG_TXDATA, in[i]);
    }
    times_out = FESPI_TX_TIMES_OUT;
    while (times_out--) {
        fespi_read_reg(spi_base, FESPI_REG_IP, &ip);
        if (ip & FESPI_IP_TXWM) {
            break;
        }
    }
    if (0 >= times_out) {
        return RETURN_FESPI_TX_ERROR;
    }
    return 0;
}

int spi_rx(uint32_t *spi_base, uint8_t *out, uint32_t len);
{
	uint32_t times_out = 0;
    uint32_t value = 0;
    uint32_t status = 0;
    fespi_set_dir(spi_base, FESPI_DIR_RX);
    for (int i = 0;i < len;i++) {
        times_out = FESPI_RX_TIMES_OUT;
        while (times_out--) {
            fespi_read_reg(spi_base, FESPI_REG_RXDATA, &value);
            if (!(value >> 31)) {
                break;
            }
        }
        if (0 >= times_out) {
            return RETURN_FESPI_RX_ERROR;
        }
        out[i] = value & 0xff;
    }
    return 0;
}
